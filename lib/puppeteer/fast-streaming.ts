...
/**
 * Setup passive network monitoring for SSE/JSON streaming.
 * Uses CDP Network.requestWillBeSent and Network.dataReceived.
 */
export async function setupNetworkMonitoring(
  ...
  client.on("Network.requestWillBeSent", (event) => {
    if (event.request.url.includes(urlPattern)) {
      console.log(`[FastStreaming] Detected request: ${event.requestId}`);
  ...
  client.on("Network.dataReceived", async (event) => {
    ...
    try {
      // Try to get response body (works best for streams in recent Chrome)
...
/**
 * Common parsers for different providers
 */
export const StreamParsers = {
  /** Standard SSE parser for ChatGPT/Claude (data: {...}) */
  sse: (text: string): string => {
